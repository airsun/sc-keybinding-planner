...
当前编辑卡片的位置、展开状态和滚动偏移只保存在本次页面会话中，不写入 workspace。搜索、状态筛选或清单切换暂时隐藏卡片时，动作重新可见后会回到原卡片和原位置；卡片隐藏期间不会把摇杆键位误绑定到该动作。
...
当前场景清单共 112 项，默认绑定 91 项。6DOF 轴与 F1/F2/F3 预填默认映射但不预锁，便于完成多场景复用后再确认锁定。

| 章节 | 绑定状态 | 配置意图 |
|---|---:|---|
| 第 1 章:登机 / 上电 | 12 / 13 | 左杆承担交互、上电、舱门和灯光；头灯暂不上杆 |
| 第 2 章:起飞 / 离港 | 13 / 14 | F1/F2/F3 预填起落架、VTOL、构型但保持可编辑；S2 放 ATC、Cargo 等流程项 |
| 第 3 章:基础飞行 / 6DOF | 12 / 13 | 六轴先核对并完成多场景复用；左 trigger 管刹车/Boost；Throttle 作为 speed limiter abs |
...
9. 搜索、筛选或切换清单后可直接返回刚才的编辑卡片；展开状态和卡片位置会随 canonical action 恢复。
10. 导出 workspace JSON 作为完整备份；导入会覆盖当前 workspace，应先确认文件来源。

## 8. 验收清单

- 6DOF 六轴在当前策略 Profile 中方向正确；完成多场景复用后可按需锁定。
